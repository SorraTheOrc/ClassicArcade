...
class TetrisState(Game):
    ...
    def run():
        ...
        # Piece state
        current_shape_name = random.choice(list(TetrisState.SHAPES.keys()))
        current_shape = TetrisState.SHAPES[current_shape_name]
        current_color = TetrisState.SHAPE_COLORS[current_shape_name]
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if not game_over:
                        if event.key == pygame.K_LEFT:
                            if TetrisState.valid_position(
                                grid, shape_coords, shape_x - 1, shape_y
                            ):
                                shape_x -= 1
                        elif event.key == pygame.K_RIGHT:
                            if TetrisState.valid_position(
                            ...
                            # Rotate clockwise
                            new_coords = TetrisState.rotate(shape_coords)
                            if TetrisState.valid_position(
                                grid, new_coords, shape_x, shape_y
                            ):
                                shape_coords = new_coords
                    else:
                        if event.key == pygame.K_r:
                            return TetrisState.run()
            # Reset fall speed after handling key press (if not holding down)
            keys = pygame.key.get_pressed()
            if not keys[pygame.K_DOWN]:
                fall_interval = FALL_SPEED
            ...
            if not game_over and fall_timer >= fall_interval:
                fall_timer = 0
                # Attempt to move piece down
                if TetrisState.valid_position(grid, shape_coords, shape_x, shape_y + 1):
                    shape_y += 1
                else:
                    # Lock piece
                    TetrisState.lock_piece(
                        grid, shape_coords, shape_x, shape_y, current_color
                    )
                    # Clear lines
                    lines = TetrisState.clear_lines(grid)
                    if lines:
                    ...
                    shape_y = 0
                    # Check immediate collision
                    if not TetrisState.valid_position(
                        grid, shape_coords, shape_x, shape_y
                    ):
                        game_over = True
            # Drawing
            screen.fill(BLACK)
